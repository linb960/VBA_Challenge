...
### Disadvantages in the Challenge refactored VBA Script
The challenge did not lead to any real disadvantages because it was limited to a small amount of data.  But if it were necessary to analyze 120 or 1200 stocks instead of just 12 the code might have caused a problem with the amount of RAM available to store the data for each ticker symbol prior to outputing the data.  The original code while having to loop hundreds of more times if a bigger dataset was given, didn't have to store that much data before it was output.  

Also with regards to complexity, this code became more complex with using variables for the tickerIndex and while writing the code began using tickersIndex and found that the code stopped working correctly.  A minor typo caused many extra hours of work.
